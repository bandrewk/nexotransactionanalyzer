...
### Added
- A rule table covering all 34 recognised transaction types, up from 22. Each type records its effect on holdings, the row shapes it was written against, whether the rule is evidenced or inferred, and an explanation shown on the File Details page. Newly recognised: `Credit Card Withdrawal Credit`, `Exchange Credit`, `Exchange Liquidation`, `Nexo Card Transaction Fee`, `Nexo Card Refund`, `Nexo Card Cashback Reversal`, `Transfer To Advanced`, `Transfer From Advanced`, `Exchange Booster`, `Exchange Collateral`, `Loan Withdrawal`, `Interest Discount`. (#84)
