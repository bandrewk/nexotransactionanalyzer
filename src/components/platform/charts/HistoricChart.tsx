...
export default function HistoricChart({ data }: Props) {
  const { colors, text, grid, tooltip } = useChartColors();
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <AreaChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
        ...
        <YAxis tick={{ fontSize: "1.1rem", fill: text }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} />
        <Tooltip
        ...
        />
        <Area type="monotone" dataKey="value" stroke={colors[0]} strokeWidth={2} fill="url(#portfolioGrad)" dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
