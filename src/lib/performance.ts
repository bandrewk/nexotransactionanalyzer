 ...
 */
const MAX_LAST_POINT_AGE_DAYS = 3;
 ...
 *
 * Returns `null` whenever the data cannot support the comparison, so the caller
 * can render "no data" instead of a number. This matters more than it looks:
...
export function computeWeekPerformance(
  ...
  if (daysBetween(last.date, today) > MAX_LAST_POINT_AGE_DAYS) return null;
  ...
  if (baseline === null || baseline === last) return null;
  if (baseline.value === 0) return null;

  const change = last.value - baseline.value;
  return { change, pct: (change / baseline.value) * 100 };
}
